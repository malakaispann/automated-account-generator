...
describe("Extraction service", () => {
	...
	const META_CONFIG = MetaConfig.readonly().parse({
		DOMAIN: DOMAIN,
		DEFAULT_ORGANIZATION: ORGANIZATION,
	});
	const PROMPT_CONFIG = PromptConfig.readonly().parse({
		FIRST_NAME_PROMPT: FIRST_NAME_KEY,
		LAST_NAME_PROMPT: LAST_NAME_KEY,
		EMAIL_ADDRESS_PROMPT: EMAIL_ADDRESS_KEY,
	});

	const extractionService = new ExtractionService(META_CONFIG, PROMPT_CONFIG);
